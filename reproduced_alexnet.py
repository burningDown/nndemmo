...
class AlexNet:
    ...
    def inference(self, images):
        with tf.name_scope("conv1"):
            # self.convs["conv1"]
            conv = tf.nn.relu(tf.nn.conv2d(images, self.weights["w1"], [1, 1, 1, 1], 'SAME') + self.bias["b1"])
    ...
    def loss(self, labels, y):
        return tf.reduce_mean(-tf.reduce_sum(labels*tf.log(y), axis=[1]))
...
def main():
    ...
    network = alex_net.inference(images)
    loss = alex_net.loss(labels, network)
